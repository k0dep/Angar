...
public class PointOctreeNode<T> where T : class {
    ...
    private Stack<PointOctreeNode<T>> objectsStack = new Stack<PointOctreeNode<T>>();
    ...
    public void Pass(ref Vector3 position, ref float maxDistance, ref Action<OctreeObject> CheckFunction)
    {
        objectsStack.Clear();

        objectsStack.Push(this);

        PointOctreeNode<T> currentOctree;
        var distance = 0f;

        while (objectsStack.Count > 0)
        {
            currentOctree = objectsStack.Pop();

            currentOctree.bounds.Expand(new Vector3(maxDistance * 2, maxDistance * 2, maxDistance * 2));
            currentOctree.bounds.size = currentOctree.actualBoundsSize;
            

            if (distance > maxDistance)
                continue;

            for (int i = 0; i < currentOctree.objects.Count; i++)
            {
                CheckFunction(currentOctree.objects[i]);
            }

            if (currentOctree.children != null)
            {
                for (int i = 0; i < 8; i++)
                {
                    objectsStack.Push(currentOctree.children[i]);
                }
            }
